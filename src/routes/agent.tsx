...
import { actionLabel, clockTime, inr, relativeTime } from "@/lib/recover-format";
  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "AI Agent Console — RecoverAI" },
      {
        ...
        content: "Ask the RecoverAI agent why an opportunity is worth recovering.",
      },
    ],
  }),
  component: AgentPage,
...
function AgentPage() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 0,
      role: "agent",
      text: "I'm monitoring your payments in real time. Ask me anything about the current opportunities — I answer only from your live data.",
    },
  ]);
  ...
  function send(text: string) {
    const q = text.trim();
    if (!q) return;
    const answer = answerAgentQuestion(q, { opportunities, metrics, customerOf });
    setMessages((prev) => [
      ...prev,
      { id: counter.current++, role: "user", text: q },
      { id: counter.current++, role: "agent", text: answer },
    ]);
    setInput("");
  }
  ...
  return (
    <AppShell>
      ...
      <div className="panel mt-5 grid divide-y divide-border sm:grid-cols-2 sm:divide-y-0 lg:grid-cols-5 lg:divide-x">
        <Stat label="Current task" value="Monitoring payments" mono={false} />
        <Stat label="Opportunities analyzed" value={String(opportunities.length)} />
        <Stat label="Actions recommended" value={String(recommended)} />
        <Stat label="Awaiting approval" value={String(awaiting)} />
        <Stat label="Recovered revenue" value={inr(metrics.recoveredRevenue)} />
      </div>
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-3">
          ...
          <div className="flex-1 space-y-4 overflow-y-auto px-5 py-4">
            {messages.map((m) => (
              <div key={m.id} className="flex gap-3">
                ...
                >
                  {m.role === "agent" ? <Bot className="size-3.5" /> : <User className="size-3.5" />}
                </span>
                <div className="min-w-0 flex-1">
                  <div className="text-xs font-medium text-muted-foreground">
                    {m.role === "agent" ? "RecoverAI Agent" : "You"}
                  </div>
                  <p className="mt-1 text-sm leading-relaxed whitespace-pre-line">{m.text}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <section className="panel p-5">
          <h2 className="text-sm font-semibold tracking-wide uppercase">Recent agent activity</h2>
          <ol className="mt-4 space-y-4">
            {audit.slice(0, 7).map((e) => (
              <li key={e.id} className="relative pl-5">
                ...
                <div className="text-xs text-muted-foreground/80" title={clockTime(e.timestamp)}>
...
function Stat({ label, value, mono = true }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="p-4">
      ...
      <div className={mono ? "tabular mt-1 text-lg font-semibold" : "mt-1 text-lg font-semibold"}>
        {value}
      </div>
    </div>
  );
