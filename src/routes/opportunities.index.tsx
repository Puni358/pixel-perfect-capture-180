...
export const Route = createFileRoute("/opportunities/")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Revenue Opportunities — RecoverAI" },
      {
        property: "og:description",
        content: "AI-ranked opportunities that could recover lost revenue.",
      },
    ],
  }),
...
const filters: { key: OpportunityType | "ALL"; label: string }[] = [
  { key: "ALL", label: "All" },
  { key: "FAILED_PAYMENT", label: opportunityTypeLabel.FAILED_PAYMENT },
  { key: "ABANDONED_CHECKOUT", label: opportunityTypeLabel.ABANDONED_CHECKOUT },
  { key: "FAILED_SUBSCRIPTION", label: opportunityTypeLabel.FAILED_SUBSCRIPTION },
];
...
function OpportunitiesPage() {
  ...
  const rows = opportunities.filter((o) => filter === "ALL" || o.opportunityType === filter);
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            AI-ranked opportunities that could recover lost revenue · {inr(metrics.revenueAtRisk)}{" "}
            at risk
          </p>
        </div>
