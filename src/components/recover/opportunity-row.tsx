...
export function OpportunityRow({
  ...
  return (
    <div className="group grid grid-cols-1 gap-4 border-b border-border px-5 py-4 transition-colors last:border-b-0 hover:bg-accent/50 lg:grid-cols-12 lg:items-center lg:gap-3">
      <div className="flex items-center gap-3 lg:col-span-3">
        ...
        <div className="min-w-0">
          ...
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            ...
            {opportunity.paymentMethod ? ` · ${opportunity.paymentMethod}` : ""}
          </div>
        </div>
      </div>

      <div className="lg:col-span-2">
        <div className="tabular text-sm font-semibold">{inr(opportunity.amount)}</div>
      ...
      <div className="lg:col-span-2">
        <Pill tone={opportunity.recommendedAction === "NO_ACTION" ? "neutral" : "info"}>
